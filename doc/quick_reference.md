...
A **domain** is a row selector: a sequence of `0`/`1` telling on which rows the
constraints attached to it must hold. Domains are declared inside an airtemplate,
with the same syntax as a fixed column sequence:

```pil
domain FIRST = [1,0...];            // only the first row
domain LAST = [0...,1];             // only the last row
domain LIMITS_16 = [1,0:14,1];      // 16 rows cycle, first and last of each block
domain LIMITS_16_EXT = [1,0:14,1]...;  // same, extended to the whole air
```

A sequence shorter than the number of rows of the air defines the cycle of a
periodic domain, exactly as periodic fixed columns do. The cycle **must be a
power of two**, every value must be `0` or `1`, and at least one row must be
selected.

A domain is attached to a single constraint with the `domain` suffix, or to a
group of statements with a `domain` block. Prefixing the name with `!` uses the
**complement** of the domain, that's the rows it doesn't select:

```pil
sel * (1 - sel) === 0 domain LIMITS_16;
a * (1 - a) === 0 domain !LIMITS_16;    // on the rows out of LIMITS_16

domain LIMITS_16 {
    enable * (1 - enable) === 0;

    // domains chain as scopes: on leaving this block we are on LIMITS_16 again
    domain FIRST {
        a === 0;
    }
    b * (1 - b) === 0;              // domain LIMITS_16
}

domain !FIRST {
    b === b';                       // every row but the first one
}

a * enable === b * sel;             // no domain, the whole air
```

Inside a domain block the innermost domain wins, and any constraint created
there (including the ones generated by called functions) gets it. Constraints
without domain hold on every row of the air. Global constraints apply to the
whole proof, so they can't have a domain.

On the pilout each air carries the `domains` it uses, stored as `cycleBits`
(the log2 of the cycle) plus the `offsets` of the cycle it selects, and its
constraints are packed grouped by domain as `allRows`, `domainRows` or
`complementDomainRows`. Only the domains used by some constraint are packed, and
their names go on the symbols (type `DOMAIN`), not on the structure.

Domains only exist on pilout version 2, the one generated by default. Compiling
with `-O pilout-version=1` generates the legacy format and fails if any constraint
has a domain.
